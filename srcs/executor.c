...
void	msh_cmd(char *line)
{
	...
	while (cmd)
	{
		...
		if (cmd->piped || cmd->input || cmd->out)
...
int	msh_buildins(t_command *cmd)
{
	int	is_buildin;

	is_buildin = 0;
	if (ft_strnstr(cmd->args[0], "exit", 4) && ++is_buildin)
		msh_custom_exit(cmd);
	else if (ft_strnstr(cmd->args[0], "pwd", 3) && ++is_buildin)
		msh_custom_pwd(cmd);
	else if (ft_strnstr(cmd->args[0], "echo", 4) && ++is_buildin)
		msh_custom_echo(cmd);
	else if (ft_strnstr(cmd->args[0], "env", 3) && ++is_buildin)
		msh_custom_env(cmd);
	else if (ft_strnstr(cmd->args[0], "cd", 2) && ++is_buildin)
		msh_custom_cd(cmd);
	else if (ft_strnstr(cmd->args[0], "export", 6) && ++is_buildin)
		msh_custom_export(cmd);
	else if (ft_strnstr(cmd->args[0], "unset", 5) && ++is_buildin)
